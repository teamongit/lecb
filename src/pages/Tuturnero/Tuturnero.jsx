...
function ListaAccionesDia({ fecha, turnos, cambiarTurno, onClose }) {
  ...
  const handleCambiarTurno = async () => {
    if (!nuevoTurno || !usuario) return;
    try {
      await cambiarTurno(fecha, usuario.nombre, nuevoTurno);
      triggerToast("Turno actualizado", { severity: "success" });
      onClose();
    } catch (err) {
      console.error(err);
      triggerToast(err.message || "Error al actualizar turno", { severity: "error" });
    }
  };
    ...
    for (let dia = 1; dia <= diasEnMes; dia++) {
      ...
      celdas.push(
        <td
          ...
          className={`position-relative p-1 ${esHoy ? "bg-warning-subtle" : ""}`}
          style={{ height: "60px", verticalAlign: "top", cursor: "pointer" }}
        >
          {/* número de día */}
          <div
            style={{
          ...
          <div            
            style={{
              ...
              backgroundColor, 
              color,
              padding: "3px 5px", // opcional, para separar texto del borde
            }}
          >
            {turnos?.[f]}
          </div>
